...
const ACTION_BUTTON = "w-8 h-8 sm:w-6 sm:h-6 rounded-md flex items-center justify-center transition-colors";
...
function rowBorder(clickable: boolean, all: boolean, some: boolean): string {
  ...
  const hover = "bg-white dark:bg-zinc-900 cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/60";
  return some ? `border-zinc-300 dark:border-zinc-600 ${hover}` : `border-zinc-200 dark:border-zinc-700 ${hover}`;
}
    ...
    >
      ...
      <div className="flex items-center gap-3 min-w-0 basis-full sm:basis-0 sm:flex-1">
        {clickable && (
          ...
          >
            {allSelectedOn && (
              <svg className="w-2.5 h-2.5 text-white dark:text-zinc-900" viewBox="0 0 10 8" fill="none">
                <path
                  d="M1 4l3 3 5-6"
                  stroke="currentColor"
                  strokeWidth="1.5"
